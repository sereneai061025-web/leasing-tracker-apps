      ...
      {(status === "expiring_soon" || status === "expired") && (
        <div className={`rounded-xl border px-5 py-3 mb-6 flex items-center gap-3 ${status === "expired" ? "bg-red-50 border-red-200" : "bg-amber-50 border-amber-200"}`}>
          ...
          <p className={`text-sm font-medium ${status === "expired" ? "text-red-700" : "text-amber-700"}`}>
            {status === "expired"
              ? `This licence expired ${Math.abs(days)} days ago — log a renewal action immediately.`
              : `This licence expires in ${days} days — log a renewal action to stay compliant.`}
          </p>
        </div>
      )}
        ...
        <div className="col-span-2 space-y-6">
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <dl className="grid grid-cols-2 gap-x-8 gap-y-4 text-sm">
              <div><dt className="text-xs text-gray-400 uppercase tracking-wide mb-0.5">Number</dt><dd className="text-gray-900 font-medium">{licence.licence_number ?? "—"}</dd></div>
              ...
              <div><dt className="text-xs text-gray-400 uppercase tracking-wide mb-0.5">Renewal Cost</dt><dd className="text-gray-900 font-semibold">{licence.renewal_cost ? `SGD ${Number(licence.renewal_cost).toLocaleString("en-SG", { minimumFractionDigits: 2 })}` : "—"}</dd></div>
              {licence.notes && (
                <div className="col-span-2"><dt className="text-xs text-gray-400 uppercase tracking-wide mb-0.5">Notes</dt><dd className="text-gray-700">{licence.notes}</dd></div>
              )}
            </dl>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            {(renewalLogs ?? []).length === 0 ? (
              <p className="px-6 py-10 text-center text-sm text-gray-400">No renewal actions logged yet. Use the form to log one.</p>
            ) : (
                ...
                {(renewalLogs as RenewalLog[]).map(log => (
                  <div key={log.id} className="px-6 py-4 flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      {log.new_expiry_date && (
                        <p className="text-xs text-emerald-600 mt-1">✓ New expiry: {new Date(log.new_expiry_date).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</p>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <div>
          <div className="bg-white rounded-xl border border-gray-200 p-5 sticky top-6">
            ...
            <form action={logRenewal} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1 uppercase tracking-wide">New Licence Number <span className="text-gray-400">(if changed)</span></label>
