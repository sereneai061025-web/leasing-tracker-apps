...
export default function MobileSidebar({ user }: { user: { email: string } | null }) {
    ...
    <>
      ...
      <div className={`md:hidden fixed top-0 left-0 bottom-0 z-50 w-64 bg-gray-900 flex flex-col transform transition-transform duration-300 ease-in-out ${open ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="px-5 py-5 border-b border-gray-700 flex items-center justify-between">
          ...
          <button onClick={() => setOpen(false)} className="text-gray-400 hover:text-white p-1 rounded-lg hover:bg-gray-800 transition-colors">
          ...
          {user ? (
            <>
              <Link href="/licences/new"
                className="flex items-center justify-center gap-2 w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-lg transition-colors">
                + Add Licence
              </Link>
              ...
              <form action="/auth/signout" method="POST">
                <button type="submit" className="w-full py-2 text-gray-500 hover:text-white hover:bg-gray-800 text-xs rounded-lg transition-colors">
                  Sign Out
                </button>
              </form>
            </>
